...
@Setter
@Schema(
    description = "Standard API response wrapper for all endpoints, providing consistent structure for success and error responses.")
public class AppApiResponse<T> {
  ...
  public static <T> AppApiResponse<T> error(String message, String path, List<ErrorDetail> errors) {
    AppApiResponse<T> response = new AppApiResponse<>();
